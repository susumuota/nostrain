...
class Relay {
  ...
  send(id: SubscriptionId, request: Message, onMessage: (response: Message) => any, onTimeout: () => any, timeout: number = -1) {
    ...
    return new Promise<any>((resolve, reject) => {
      const listener = (ev: MessageEvent) => {
        try {
          ...
          if (result !== undefined) { // TODO: is it OK to use undefined like this?
  ...
  subscribe(filters: Filter[], onEvent: (event: Event) => any, onEOSE: () => any, onTimeout: () => any, timeout: number = -1) {
  ...
  list(filters: Filter[], timeout: number = -1) {
    const events: Event[] = [];
    const onEvent = (event: Event) => { events.push(event); return undefined; };
    return this.subscribe(filters, onEvent, () => events, () => events, timeout);
  }

  get(filters: Filter[], timeout: number = -1) {
    return this.subscribe(filters, event => event, () => null, () => null, timeout);
  }
